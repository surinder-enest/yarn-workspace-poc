...
class MobilePage extends React.Component<Props> {
    private getBuilderElement(builderElement: BuilderElementModel): ReactNode {
        ...
        const { moduleId, contactId, accountId, responseCapturedFromModule, isActualRendering } = this.props;
        switch (builderElement.builderElementType) {
            case BUILDER_ELEMENTS.TITLE:
                return <Title key={builderElement.key} builderElement={builderElement} />;
    ...
    private getBuilderSectionAction(builderElement: BuilderElementModel): ReactNode {
