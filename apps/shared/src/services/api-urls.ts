export const apiUrl = {
  //MOBILE PAGE
  getMobilePageDetailsForRender:
    'api/MobilePage/GetMobilePageDetailsForSSRRender',
  saveMobilePageOpenDetails: 'api/MobilePage/SaveMobilePageOpenDetails',
  //BUILDER ELEMENT
  saveBuilderElementResponse: 'api/BuilderElementResponse/SaveResponse',
  getBuilderElementDetailForSnapShot:
    'api/BuilderElement/GetBuilderElementDetailsForSnapshot',
  //CONTACT CAPTURE
  saveContactCapture: 'api/ContactCapture/SaveContactCapture',
  //OFFER
  saveContactAndRedeemOffer: 'api/Offer/SaveContactAndRedeemOffer',
  //WEB FORM
  getWebFormDetailById: 'api/WebForm/RenderWebFormDetailById',
  webFormContactCapturingResponse: 'api/WebForm/WebFormContactCapturingResponse',
};
