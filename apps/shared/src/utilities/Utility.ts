...
export class Utility {
  ...
  public static addStringBeforeCapitalLetter(
    str: string,
    join: string
  ): string {
  ...
  public static redirectToOtherPage = (
    redirectUrl: string,
  ...
  public static getWistiaUrl(url: string): string {
    ...
    if (parsed)
      return '//fast.wistia.net/embed/iframe/' + parsed[0].split('/')[2];
    return '';
  }
  ...
  public static getEmbedWistiaUrl(url: string): string {
    ...
    if (parsed) {
      ...
      return (
        '//fast.wistia.net/embed/iframe/' + embededCode[0].split('/medias/')[1]
      );
    }
    return '';
  }
  ...
  public static getValueWithClickType(
    type: string,
    value: string,
    redirectUrl: string,
    contactId: string
  ...
  public static formatDateInMMDDYY(beginDate: string): string {
  ...
  public static secondsToHHMM12HourFormat(microSeconds: number): string {
    let d = Number(microSeconds);
